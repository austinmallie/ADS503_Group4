...
# Extract MONTH factor levels from the model
...
server <- function(input, output) {
  observeEvent(input$predict_btn, {
    ...
    # Convert month input to factor with correct levels
    month_val <- as.character(input$month)
    
    if (!(month_val %in% month_levels)) {
      month_val <- month_levels[1]  # fallback if level not seen before
    }
    ...
    new_data <- data.frame(
      ...
      YEAR = input$year,
      MONTH = factor(month_val, levels = month_levels)
    )

    # Debug: view input data
...
```{r Shiny}
# Logistic Regression
log_model <- glm(high_bleaching ~ avg_sst + DEPTH_CLEAN + YEAR + MONTH,
                 data = model_data, family = "binomial")

# Decision Tree
library(rpart)
tree_model <- rpart(high_bleaching ~ avg_sst + DEPTH_CLEAN + YEAR + MONTH,
                    data = model_data, method = "class")

# Random Forest
library(randomForest)
rf_model <- randomForest(high_bleaching ~ avg_sst + DEPTH_CLEAN + YEAR + MONTH,
                         data = model_data, ntree = 100)
```


```{r Shiny}
# Get factor levels from trained logistic regression model (used as reference)
month_levels <- levels(model.frame(log_model)$MONTH)

# UI definition
ui <- fluidPage(
  titlePanel("Coral Bleaching Risk Predictor"),
  sidebarLayout(
    sidebarPanel(
      selectInput("model_type", "Select Model:",
                  choices = c("Logistic Regression", "SVM", "Ridge", "Lasso",
                              "Elastic Net", "LDA", "Linear Regression", "PLS", "KNN")),
      numericInput("sst", "Average SST:", value = 29, min = 24, max = 34),
      numericInput("depth", "Depth (m):", value = 10, min = 0, max = 100),
      numericInput("year", "Year:", value = 2022, min = 1980, max = 2030),
      numericInput("month", "Month (1-12):", value = 6, min = 1, max = 12),
...
server <- function(input, output) {
  observeEvent(input$predict_btn, {
    ...
    prob <- switch(model_name,
      "Logistic Regression" = predict(log_model, newdata = new_data, type = "response"),
      "SVM" = predict(svm_model, newdata = new_data),
      "Ridge" = predict(ridge_model, newx = model.matrix(~., new_data)[,-1]),
      "Lasso" = predict(lasso_model, newx = model.matrix(~., new_data)[,-1]),
      "Elastic Net" = predict(enet_model, newx = model.matrix(~., new_data)[,-1]),
      "LDA" = predict(lda_model, newdata = new_data)$posterior[,2],
      "Linear Regression" = predict(lm_model, newdata = new_data),
      "PLS" = predict(pls_model, newdata = new_data),
      "KNN" = predict(knn_model, newdata = new_data)
    )
